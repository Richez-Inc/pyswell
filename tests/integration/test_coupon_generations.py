...
@vcr.use_cassette(
    "tests/vcr_cassettes/generations/test-list-generations-with-limit.yml"
)
...
def test_get_generations_by_coupon_id(swell, generation_keys):
    ...
    assert set(generation_keys).issubset(
        id_response.keys()
...
def test_create_coupon_generation(swell, generation_keys):
    ...
    new_coupon_generation = {"parent_id": first_coupon_id, "count": 50}

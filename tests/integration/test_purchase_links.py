...
@vcr.use_cassette(
    "tests/vcr_cassettes/purchase_links/test-purchase-links-date-filter.yml"
)
...
@vcr.use_cassette(
    "tests/vcr_cassettes/purchase_links/test-list-purchase-links-with-limit.yml"
)
...
def test_get_purchase_link_by_id(swell, purchase_link_keys):
    ...
    assert set(purchase_link_keys).issubset(
        id_response.keys()
...
def test_update_purchase_link(swell, purchase_link_keys):
    ...
    updates = {"id": first_purchase_link_id, "name": "new name 2"}

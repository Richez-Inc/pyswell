...
def test_get_coupon_by_id(swell, coupon_keys):
    ...
    assert set(coupon_keys).issubset(
        id_response.keys()
...
def test_create_coupon(swell, coupon_keys):
    ...
    new_coupon = {
        "name": "99% Off Sale",
        "active": False,
        "discounts": {"value_type": "percent", "value_percent": 90},
        "codes": [{"code": "SAVE98"}],
    }
...
def test_fails_with_no_discounts(swell):
    ...
    with pytest.raises(ValueError):
        swell.coupons.create({"codes": [{"code": "SAVE90"}], "name": "test"})
...
def test_update_coupon(swell, coupon_keys):
    ...
    updates = {"id": first_coupon_id, "name": "new coupon name"}
